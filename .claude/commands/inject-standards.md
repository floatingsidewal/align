...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `align/standards/` (not in a subfolder).
...
Read `align/standards/index.yml` to get the list of available standards and their descriptions.
...
@align/standards/api/response-format.md
@align/standards/api/error-handling.md
@align/standards/global/naming.md
...
Arguments can be:
- **Folder name** — `api` → inject all `.md` files in `align/standards/api/`
- **Folder/file** — `api/response-format` → inject `align/standards/api/response-format.md`
- **Root folder** — `root` → inject all `.md` files directly in `align/standards/` (not in subfolders)
- **Root file** — `root/naming` → inject `align/standards/naming.md`
